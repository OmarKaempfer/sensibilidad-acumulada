...
def matches_criteria(dataframe, antibiotics, excluded=None, included=None):
    ...
    for index, row in dataframe.iterrows():
        ...
        if (excluded is not None) and helper.get_key_matches(excluded, microorganismo_name):
            continue
        if (included is not None) and not helper.get_key_matches(included, microorganismo_name):
            continue
...
def get_sensibility_percentages(df):
    ...
    for index, row in df.iterrows():
        if helper.get(row, 'microorganismo') in microorganisms:
            microorganism_record = microorganisms.get(helper.get(row, 'microorganismo'))
        else:
            microorganism_record = initialize_microorganism_record(row, antibiotics)
            microorganisms[helper.get(row, 'microorganismo')] = microorganism_record
